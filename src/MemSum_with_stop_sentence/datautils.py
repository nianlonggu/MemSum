...
class Vocab:
    def __init__(self, words, eos_token = "<eos>", pad_token = "<pad>", stop_token = "<unk>" ):
        self.words = words
        self.index_to_word = {}
        self.word_to_index = {}
        ...
        self.stop_index = self.word_to_index[self.stop_token]

        self.tokenizer = SentenceTokenizer()   

    def index2word( self, idx ):
        return self.index_to_word.get( idx, "")
...
class ExtractionTrainingDataset(Dataset):
    ...
    def __getitem__( self, idx ):
        ...
        ## add a STOP sentence at the very beginning.
        sentences = [ self.vocab.stop_token ] + sentences
        ## shift all indices by one to the right
        indices = [  np.array(item) + 1 for item in  indices ]
        ...
        valid_sen_idxs = valid_sen_idxs[ valid_sen_idxs < min(num_sentences_in_doc, self.max_doc_len ) ]
        selected_y_label = np.zeros( num_sentences_in_doc )
        selected_y_label[ valid_sen_idxs ] = 1
        ## pos 0 is always selected
        selected_y_label[0] = 1

        selected_score = scores[ rand_idx ]
        

        valid_sen_idxs = valid_sen_idxs[:self.max_doc_len-1]

        # add pos 0 to the last valid sen idxs
        valid_sen_idxs = np.array(valid_sen_idxs.tolist() + [0] + [-1] * ( self.max_doc_len - (len(valid_sen_idxs)+1)  )  )
