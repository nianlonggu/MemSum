...
def train_iteration(batch):
    ...
    for step in range(valid_sen_idxs.shape[1]):
        ...
        p, _, baseline = extractor( local_sen_embed, global_context_embed, extraction_context_embed , extraction_mask , dropout_rate )
        
        
        sen_indices = valid_sen_idxs[:, step]
        done = torch.logical_or(sen_indices == -1, sen_indices == 0   )
        ...
        log_action_prob = normalized_p.masked_fill(   torch.logical_and( done, ~just_stop  )    , 1.0 ).log()
        
        log_action_prob_list.append( log_action_prob.unsqueeze(1) )

        done_list.append(done)
        just_stop_list.append( just_stop.unsqueeze(1) )
    ...
    log_prob_list = torch.cat( log_action_prob_list, dim = 1 )

    done_list = torch.cat( [ done_i.unsqueeze(1) for done_i in done_list  ], dim = 1  )

    just_stop_list = torch.cat(just_stop_list, dim = 1  )
    
    # reward_weight = torch.ones_like( just_stop_list ).masked_fill(just_stop_list, 0.01)
    # loss = -(log_prob_list * reward_weight * selected_score.unsqueeze(1)).sum(dim =1) #/ ( torch.ones_like( done_list ).masked_fill(done_list, 0 ).sum(dim=1) + 1e-9  )
    
    loss = -(log_prob_list * selected_score.unsqueeze(1)).sum(dim =1)
...
def validation_iteration(batch):
    ...
    for step in range(max_extracted_sentences_per_document ):
        ...
        p, _, baseline = extractor_ema( local_sen_embed, global_context_embed, extraction_context_embed , extraction_mask  )
        ...
        for doc_i in range( num_documents ):
            if  len( done_list )==0 or not done[doc_i]:
                sen_i = sen_indices[ doc_i ].item()
                remaining_mask_np[doc_i,sen_i] = False
                extraction_mask_np[doc_i,sen_i] = True
    ...
    for doc_i in range(seqs.shape[0]):
        ...
        ## drop the stop sentence if exists
        extracted_sen_indices = extracted_sen_indices[ extracted_sen_indices != 0 ]
